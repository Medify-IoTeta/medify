// Medify - FULL Collision Check: Floor0 + Floor1 + Floor2 + Door + Components
$fn = 100;
...
module floor2_part() {
    translate([0, 0, floor0_height + floor1_height]) {
        include <Floor2Base4FINAL.scad>
    }
}
...
color("red") {
    ...
    intersection() { floor1_part(); floor2_part(); }

    intersection() { floor1_part(); door_part(); }
    intersection() { floor0_part(); door_part(); }
    intersection() { floor2_part(); door_part(); }

    intersection() { floor1_part(); breadboard_part(); }
    intersection() { floor1_part(); arduino_part(); }
    intersection() { floor1_part(); fan_part(); }

    intersection() { floor2_part(); breadboard_part(); }
    intersection() { floor2_part(); arduino_part(); }
    intersection() { floor2_part(); fan_part(); }
